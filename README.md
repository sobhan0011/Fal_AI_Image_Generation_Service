...
Service publicly available on: https://fal-ai-image-generation-service.onrender.com/

`POST /generate/image` is protected with a small demo key (given in email) so random visitors cannot spend fal.ai credits.

The root redirects to Swagger:

```text
https://fal-ai-image-generation-service.onrender.com/docs
```
...
```bash
curl -X POST "https://fal-ai-image-generation-service.onrender.com/generate/image" \
